...
3. 类型转换
   ...
   * 转字符型
     ...
     // toString 方法，注意 null 和 undefined没有toString 方法
     console.log(num1.toString(2)) // 此时输出结果为11010，即先把数字转换为二进制，再转换为字符；默认不用写2，为十进制转换
     ```
...
## 11.DOM对象（Document Object Model）
...
6. DOM对象的继承关系
   ...
   | document.getElementByTagName()   | 根据标签名查找元素                    |
   | document.getElementByName()      | 根据指定的name查找元素                |
   | document.getElementByClassName() | 根据类名查找元素                      |
   ...
   | 属性        | 说明                                                         |
   | ----------- | ------------------------------------------------------------ |
   | innerHTML   | 设置或者返回两个标签之间的html内容  **=>** 推荐使用该方法进行元素的内容设置 |
   | innerText   | 设置或者返回元素中去掉所有标签后的内容                       |
...
```javascript
document.fisrtChild				//返回值为 <!DOCTYPE html>	
