...
是指在一个事务内，多次读同一数据。在这个事务还没有结束时，另外一个事务也访问该同一数据。那么，在第一个事务中的两次读数据之间，由于第二个事务的修改，那么第一个事务两次读到的的数据可能是不一样的。这样就发生了在**一个事务内两次读到的数据是不一样的**，因此称为是不可重复读。（即不能读到相同的数据内容）
...
- 在事务1查询结束后，但是事务1并没有结束，即没有提交；

  事务2往User表中插入了一条id为1的数据，并完成提交。

  ```mysql
  insert into `User`(`id`, `name`) values (1, 'Joonwhee');
  ```

- 此前，由于事务1查询到id为1的用户不存在，因此插入1条id为1的数据。
...
目前工资为1000的员工有10人。事务1，读取所有工资为1000的员工，共读取10条记录 
...
这时事务2向employee表插入了一条员工记录，工资也为1000,并完成了提交。
...
只不过**幻读是重点在插入和删除(insert、delete)**，**不可重复读重点在修改(update)**
...
​		存储过程（Stored Procedure）和函数是实现经过编译并存储在数据库中得一段SQL语句的集合，调用存储过程和函数可以简化应用开发人员的很多工作，减少数据在数据库和应用服务器之间的传输，提高数据处理的效率。

​		存储过程和函数的区别：

​	  存储过程：没有返回值的函数。	函数：有返回值的过程； 

<img src="..\_posts\images\image-20210719101124056.png" alt="image-20210719101124056" style="zoom: 80%;" />
...
```mysql
CREATE PROCEDURE  procesure_name
begin
	--sql语句集合
end;
```
...
# 查询某个存储过程的定义
show create procedure test.protest_01 \G;
```
...
* DECLARE
  ...
  通过declare声明一个局部变量，介于begin和end之间
...
begin
  declare num int;
  select count(*) into num from city ;	# 存储过程到变量中
end$
delimiter ;
```
...
CREATE PROCEDURE pro_test3()
begin 
  declare num int default 0;		# 声明变量类型
...
begin
  declare num int;
  select count(*) into num from city ;   # 存储过程的用法，结合into到变量中
end$
delimiter ;
```
...
触发器是与表有关的数据库对象，指在insert/update/delete之前或之后，触发并执行触发器中定义的SQL语句集合。触发器的这种特性可以协助应用在数据库端确保数据的完整性，日志记录和数据校验等操作。

使用别名`OLD`和`NEW`来引用触发器中发生变化的记录内容，这与其他的数据库是相似的。Mysql现在触发器还只支持行级触发，不支持语句级触发,所以mysql需要加上`FOR EACH ROW`
